-- Coupon System with First Order Coupon
-- This script creates a comprehensive coupon system with automatic first order coupon
...
-- Step 2: Create coupon_usage table to track usage
...
-- Step 3: Add coupon information to orders table
DO $$ 
BEGIN
...
-- Step 4: Create default first order coupon
INSERT INTO coupons (
  code,
  name,
  description,
...
-- Step 5: Create function to check if customer is first-time
...
-- Step 6: Create function to get applicable coupons
...
-- Step 7: Create function to apply coupon
...
-- Step 8: Create function to get first order coupon automatically
...
-- Step 9: Create trigger to update coupon usage count
...
-- Step 10: Create trigger for coupon usage tracking
...
COMMENT ON TABLE coupons IS 'Coupon system for discounts and promotions';
COMMENT ON TABLE coupon_usage IS 'Tracks coupon usage by customers';
COMMENT ON COLUMN coupons.is_first_order_only IS 'Whether this coupon is only for first-time customers';
COMMENT ON COLUMN orders.coupon_id IS 'Applied coupon ID';
COMMENT ON COLUMN orders.coupon_code IS 'Applied coupon code';
COMMENT ON COLUMN orders.coupon_discount IS 'Discount amount from coupon';

-- Step 13: Test the system
SELECT 'Coupon system setup completed successfully!' as status;

-- Step 14: Show default first order coupon
SELECT 
  code,
  name,
  description,
